define([
  ...
  var TweetAvatarLink = React.createClass({
    mixins: [TweetMixin],
    render: function () {
      ...
      return (
        <div className="tweet-avatar">
          <a href={userProfileLink}>
            <Image url={userProfileImageURL} width="48px" height="48px" style={avatarStyle} />
          </a>
        </div>
      );
    }
  ...
  var TweetContentHeader = React.createClass({
    mixins: [TweetMixin],
    render: function () {
      var userProfileLink = this.getUserProfileLink();
      ...
      return (
        <div className="tweet-content-user-header">
          <a className="profile-link" href={userProfileLink}>
            <span className="user-fullname" style={userFullNameStyle}>{userFullName}</span>&nbsp;
            <span className="user-screenname" style={userScreenNameStyle}>{userScreenName}</span>
          </a>
          <span className="tweet-timeago" style={tweetTimeAgoStyle}>&nbsp;·&nbsp;
            <a className="tweet-link" href={tweetLink} style={tweetTimeAgoStyle}>{tweetTimeAgo}</a>
          </span>
        </div>
      );
    }
  ...
  var TweetBody = React.createClass({
    ...
    render: function () {
      ...
      if (photoForDisplay) {
        ...
        var image = <Image url={photoForDisplay.url} style={imageStyle}/>;
        tweetMedia = <div className="tweet-media" style={tweetMediaStyle}>{image}</div>
      ...
      if (!this.props.collapsed) {
        ...
        bodyFooter = <div className="tweet-body-footer" style={bodyFooterStyle}>
          {createdTimestamp}&nbsp;·&nbsp;
          <a href={tweetLink} style={tweetLinkStyle}>Details</a>
          </div>;
      }

      return (
        <div>
          <div className="tweet-text" style={tweetTextStyle}
  ...
  var TweetContent = React.createClass({
    ...
    render: function () {
      ...
      return (
        <div className="tweet-content" style={tweetContentStyle}>
          <TweetContentHeader   tweet={this.props.tweet} />
          <TweetBody            tweet={this.props.tweet} collapsed={this.props.collapsed} />
          <div className="tweet-content-footer-parent" style={tweetContentFooterStyle}>
            <TweetActions.footer  tweet={this.props.tweet} />
          </div>
        </div>
      );
    }
  ...
  var Tweet = React.createClass({
    ...
    render: function () {
      ...
      return (
        <div className="tweet-item-parent" style={tweetItemParentStyle} onClick={this.handleClick}>
          <TweetAvatarLink  tweet={this.props.tweet} />
          <TweetContent     tweet={this.props.tweet} collapsed={this.state.collapsed} />
        </div>
      );
    }
  });
