  ...
  var TweetBody = React.createClass({displayName: 'TweetBody',
    ...
    render: function () {
      ...
      if (photoForDisplay) {
        ...
        var tweetMediaClass = "tweet-media transition-1s";

        if (this.props.collapsed) {
          //tweetMediaStyle.maxHeight = "253px";
          tweetMediaClass += " max-height-photo-small";
          tweetMediaStyle.borderRadius = "5px";

          imageStyle = { marginTop: "-91px" };
        } else {
          tweetMediaClass += " max-height-photo-full";
        }

        var image = React.createElement(Image, {url: photoForDisplay.url, style: imageStyle});
        tweetMedia = React.createElement("div", {className: tweetMediaClass, style: tweetMediaStyle}, image)
